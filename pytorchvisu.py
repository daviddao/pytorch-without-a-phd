...
plt.style.use(["ggplot", "pytorchvisu.mplstyle"])
#import matplotlib
...
# X: tensor of shape [100+, 1, 28, 28] containing a batch of images (float32)
# Y: tensor of shape [100+, 1] containing recognised label (int)
# Y_: tensor of shape [100+, 1] containing correct digit labels (int)
...
def numpy_format_mnist_images(X, Y, Y_, n=100, lines=10):
    correct_predictions = (Y == Y_)  
    correctly_recognised_indices = np.where(Y == Y_)[0] # get all the indices where correctly predicted
    incorrectly_recognised_indices = np.where(Y != Y_)[0] # get all the indices which where incorrectly predicted
    everything_incorrect_first = np.concatenate([incorrectly_recognised_indices, correctly_recognised_indices]) # incorrect first
    everything_incorrect_first = everything_incorrect_first[:n] # get the first n only
    Xs = X[everything_incorrect_first]
    Ys = Y[everything_incorrect_first]
    Ys_ = Y_[everything_incorrect_first]
    correct_predictions_s = correct_predictions[everything_incorrect_first]
   
    # background filter
    correct_bkg = np.tile([1.3,1.3,1.3], [28*28]).reshape((1,28,28,3)) # white background
    incorrect_bkg = np.tile([1.3,1.0,1.0], [28*28]).reshape((1,28,28,3)) # red background
    choice_bkg = np.concatenate([incorrect_bkg, correct_bkg], axis=0) # concatenate both images
    recognised_bkg = np.array([choice_bkg[i] for i in correct_predictions_s.astype(int)]) # pick either red or white background
    
    I = np.concatenate([Xs] * 3, axis=1).copy(order='C') # create RGB
    I = (np.transpose((1-I),(0,2,3,1))*recognised_bkg)/1.3 #negative, transpose to (B,W,H,C) format and then add background 
...
class MnistDataVis:
    ...
    def animate(self, compute_step, iterations, train_data_update_freq=20, test_data_update_freq=100, one_test_at_start=True, more_tests_at_start=False, save_movie=False):
        ...
        if save_movie:
            ...
            self._animation.save("./pytorchvisu_video.mp4", writer=mywriter)
        else:
            plt.show(block=True)
